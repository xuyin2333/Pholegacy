...
float blocky_clouds_density(
    ...
    if (rotate) {
        world_pos.xz = vec2(-world_pos.z, world_pos.x);
    }
    world_pos.xz += wind_velocity * world_age;

    // Minecraft cloud noise

    float size_mult = rotate ? 1.4 : 1.0;
    float density = texture_soft(
        depthtex2,
        world_pos.xz * rcp(BLOCKY_CLOUDS_SIZE * 5000 * size_mult),
        roundness
    );
    ...
    float cloud_density = clamp01(density) * BLOCKY_CLOUDS_DENSITY;
    cloud_density *= 1.0 - wetness * 0.90;
...
float blocky_clouds_optical_depth(
    ...
    for (uint i = 0u; i < step_count; ++i, ray_pos += ray_step.xyz) {
        ...
        optical_depth
            += blocky_clouds_density(world_pos, altitude_fraction, layer_offset, rotate)
            * ray_step.w;
    }
...
vec4 raymarch_blocky_clouds(
    ...
    for (uint i = 0u; i < primary_steps; ++i, world_pos += world_step) {
        ...
        float density
            = blocky_clouds_density(world_pos, altitude_fraction, layer_offset, is_rotated);
        if (density < eps) {
            continue;
        }
