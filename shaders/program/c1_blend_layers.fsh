...
#define is_translucent_lod false
    ...
    bool is_rain_marker = refraction_data.a > 0.9 && length(refraction_data.rgb) < 0.01;

#if REFRACTION != REFRACTION_OFF
    if (is_translucent && !is_rain_marker && refraction_data != vec4(0.0)) {
...
// Rain/snow post-processing: SSS backlight + bloom interaction.
// Detection via colortex3 marker (is_rain_marker, independent of RAIN_OPACITY).
// Main rain rendering is handled by blend_layers_with_fog above — this block
// only adds subtle light interaction that won't break with transparency changes.
#ifdef WORLD_OVERWORLD
    if (is_rain_marker && rainStrength > 0.01) {
        // Very subtle SSS backlight: water droplets scatter light forward
        // when viewing towards light source. Low g=0.2 keeps the peak gentle
        // so rain doesn't glow unnaturally when facing the sun.
